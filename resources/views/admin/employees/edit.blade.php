        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    @if (session('success'))
                        <div class="mb-4 p-4 text text-green-700 bg-green-100 rounded-lg" role="alert">
                            {{ session('success') }}
                        </div>
                    @endif
                    @if (session('error'))
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="data_nascimento" class="block mt-1 w-full" type="date"
                                    name="data_nascimento" :value="old(
                                        'data_nascimento',
                                        \Carbon\Carbon::parse($employee['data_nascimento'])->format('Y-m-d'),
                                    )" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="data_admissao" class="block mt-1 w-full" type="date"
                                    name="data_admissao" :value="old(
                                        'data_admissao',
                                        \Carbon\Carbon::parse($employee['data_admissao'])->format('Y-m-d'),
                                    )" required />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                            <div>
                                ...
                                <select name="type_id" id="type_id"
                                    ...
                                    <option value="{{ $typeEmployee['id'] }}" @selected(old('type_id', $typeEmployee['id']) == $typeEmployee['id'])>
                                        {{ $typeEmployee['nome'] }}
                                    </option>
                                </select>
